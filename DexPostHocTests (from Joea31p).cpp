...
// Movement triggers must be marked by MarkEvent( TRIGGER_MOVEMENT );
// 
...
int DexApparatus::CheckEarlyStarts(  int max_early_starts, float hold_time, float threshold, float filter_constant, const char *msg ) {
	...
	int		early_starts = 0, movements = 0;
	...
	// First we should look for the start and end of the actual movement based on 
	// events such as when the subject reaches the first target. 
	FindAnalysisFrameRange( first_frame, last_frame );

	// Compute the instantaneous tangential velocity. 
	tangential_velocity[first_frame] = 0.0;
	...
	if ( N <= 0.0 ) {
		...
		for ( frm = first_frame; frm < last_frame; frm++ ) {
		...
		for ( frm = last_frame - 2; frm >= first_frame; frm-- ) {
			tangential_velocity[frm] = ( filter_constant * tangential_velocity[frm+1] + tangential_velocity[frm] ) / ( 1.0 + filter_constant );
		}
		...
		for ( i = first_event; i < last_event; i++ ) {
			if ( eventList[i].event == TRIGGER_MOVEMENT ) {
				...
				for ( j = index; j > index - hold_frames && j > first_frame; j-- ) {
					if ( tangential_velocity[j] > threshold ) {
						early_starts++;
						break;
					}
				}
	...
	if ( error ) {
		...
		int response = fSignalError( MB_ABORTRETRYIGNORE, fmt, msg, movements, early_starts, max_early_starts );
	...
	// This is my means of signalling the event.
	monitor->SendEvent( "Early Starts OK.\n  Total Movements: %d\n  False Starts Detected: %d\n  Maximum Allowed: %d", movements, early_starts, max_early_starts );
	return( NORMAL_EXIT );
