...
int RunCollisions( DexApparatus *apparatus ) {
	...
	for ( int target = 0; target < collisionSequenceN; target++ ) {
		...
		// Allow a fixed time to reach the starting point.
		apparatus->Wait( movementTime );
				
		apparatus->TargetsOff();
		if ( collisionSequence[target] ) {
			// This is to check for errors in movement direction.
			apparatus->MarkEvent( TRIGGER_MOVE_UP );
			// This is to detect false starts and the starting position.
	...
	status = apparatus->CheckEarlyStarts( 1, 0.100, 100.0, 1.0 );
	if ( status == ABORT_EXIT ) exit( ABORT_EXIT );
	status = apparatus->CheckCorrectStartPosition( collisionInitialTarget, 10.0, 20.0, 20.0, 1 );
	if ( status == ABORT_EXIT ) exit( ABORT_EXIT );
