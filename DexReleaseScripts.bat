...
echo Trying to create tar of Proofs.
pushd %PROOFS%
echo nothing __dummy.bmp
del /Q *.bmp
popd
...
pushd %PROOFS%
%MD5% *.bmp > Proofs.md5
%TAR% --create --verbose --file=Proofs.tar *.bmp Proofs.md5
popd
move %PROOFS%\Proofs.tar Proofs.tar
copy /Y /V Proofs.tar "%DESTINATION%\%TIMESTAMP% Proofs.tar"
echo Finished creating tar of proofs.
goto NEXT
