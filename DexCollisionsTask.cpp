...
int PrepCollisions( DexApparatus *apparatus, const char *params ) {
	...
	AddDirective( apparatus, "You will hold the manipulandum upright and to the right of the blinking Target LED.", "MvToBlkTap.bmp" );
...
int RunCollisions( DexApparatus *apparatus, const char *params ) {
	...
	apparatus->WaitUntilAtVerticalTarget( collisionInitialTarget, desired_orientation, defaultPositionTolerance, defaultOrientationTolerance, waitHoldPeriod, waitTimeLimit, MsgTooLongToReachTarget, "MvToBlkTap.bmp" );
	if ( status == ABORT_EXIT ) exit( status );
